-- Lunara fixed: top-level locals promoted to globals (Luau 200-register limit)
-- Freeze fix: heavy AC scans deferred + yielded (feature logic kept)
getgenv().LunaraRunning = true

-- Lightweight metatable hook only (cheap)
pcall(function()
...
-- Deferred AC work (does not block menu load)

-- Full anti-cheat system (Lunara logic, deferred to avoid freeze)
task.spawn(function()
    task.wait(2.5)
    -- Disable known AC LocalScripts / ModuleScripts
    pcall(function()
        local tags = {"anticheat","ac","detection","ban","kick","security","moderation","anticheatcontroller","anti-cheat"}
        local function proc(o)
            pcall(function()
                if not o or not o:IsA("LuaSourceContainer") then return end
                local ok, nm = pcall(function() return o.Name:lower() end)
                if not ok or not nm then return end
                for i = 1, #tags do
                    if nm:find(tags[i], 1, true) then
        ...
        pcall(function()
            ...
            if rs then table.insert(roots, rs) end
        end)
        ...
        pcall(function()
            game.DescendantAdded:Connect(function(o) task.defer(proc, o) end)
        end)
    end)
    -- NetworkClient AC child kill
    pcall(function()
        local nc = game:GetService("NetworkClient")
        if not nc then return end
        nc.ChildAdded:Connect(function(ch)
            pcall(function()
                local ok, n = pcall(function() return ch.Name:lower() end)
                if ok and n and (n:find("anticheat", 1, true) or n:find("detection", 1, true) or n:find("security", 1, true)) then
    ...
    -- Hook ban/kick constants in LoadingScreen / LocalScript3 (getgc)
    task.wait(0.8)
    pcall(function()
        if type(getgc) ~= "function" then return end
        local rf = game:GetService("ReplicatedFirst")
        local tgt = rf:FindFirstChild("LocalScript3")
            ...
            if type(fn) == "function" then
                ...
                if ok1 and type(env) == "table" then
                    ...
                    if ok2 and scr and typeof(scr) == "Instance" then
                        ...
                        if ok3 and (scr == tgt or (type(ss) == "string" and ss:find("LoadingScreen"))) then
                            local ok4, consts = pcall(debug.getconstants, fn)
                            if ok4 and type(consts) == "table" then
                                for j = 1, #consts do
                                    local c = consts[j]
                                    if type(c) == "string" and (c:find("TakeTheL") or c:find("ban") or c:find("kick") or c:find("Ban") or c:find("Kick")) then
    ...
    -- Namecall / index protection stubs (safe no-ops if unavailable)
    pcall(function()
...
-- ============================================================
-- 在載入任何 Linoria / NOIR UI 之前，先等競爭者 UI
-- 避免 NOIR 先建立 ScreenGui 把對方搶掉
-- ============================================================
do
    local CoreGui = game:GetService("CoreGui")
    local Players = game:GetService("Players")
    local ROBLOX_CORE = {
        RobloxGui = true, TopBarApp = true, InspectorView = true,
        PlayerList = true, Chat = true, BubbleChat = true,
        Cursor = true, CursorGui = true, DevConsoleMaster = true,
        GamepadMenu = true, CaptureControl = true, PurchasePrompt = true,
        TeleportGui = true, ScreenTimeShutdown = true, ExperienceShutdown = true,
        RobloxPromptGui = true, NotificationFrame = true, CoeurApp = true,
    }
    local function foreignList()
        local t = {}
        pcall(function()
            for _, c in ipairs(CoreGui:GetChildren()) do
                if c:IsA("ScreenGui") then
                    ...
                    if not ROBLOX_CORE[n]
                        and not low:find("noir", 1, true)
                        and not low:find("linoria", 1, true)
                        and not low:find("instance", 1, true)
                    then
                        table.insert(t, n)
                    end
                end
    ...
    print("[NOIR] pre-wait: looking for competitor UI before loading Linoria...")
    local deadline = os.clock() + 45
    local lastKey, stable = "", 0
    local found = false
    while os.clock() < deadline do
        local list = foreignList()
        ...
        if #list >= 1 then
            if key == lastKey then
                ...
                if stable >= 12 then
                    found = true
                    print("[NOIR] pre-wait OK · competitor UI:", key)
                    break
                end
            else
                stable = 0
                lastKey = key
                print("[NOIR] pre-wait saw UI:", key)
            end
        else
            stable = 0
            lastKey = ""
    ...
    if not found then
        print("[NOIR] pre-wait timeout · loading NOIR anyway (no foreign CoreGui)")
        -- 至少等 PlayerGui 一點
        pcall(function()
...
-- Reliable notify (Linoria + system fallback, no recursion)
_noirLastNotifyAt = 0
_noirLastNotifyMsg = ""
function NoirNotify(msg, dur)
    msg = tostring(msg or "")
    dur = tonumber(dur) or 3
    local now = os.clock()
    -- de-dupe spam (same message within 0.8s)
    if msg == _noirLastNotifyMsg and (now - _noirLastNotifyAt) < 0.8 then
        return false
    end
    _noirLastNotifyAt = now
    _noirLastNotifyMsg = msg
    local ok = false
    pcall(function()
        if Library and type(Library.Notify) == "function" then
            -- call method without using Library:Notify (name may be shadowed)
            Library.Notify(Library, msg, dur)
            ok = true
        end
    end)
    if not ok then
...
getgenv().NoirNotify = NoirNotify

UserInputService = game:GetService("UserInputService")
RunService = game:GetService("RunService")
IS_MOBILE = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled and not UserInputService.MouseEnabled
function isMobile()
...
visibilityCache = {}
VISIBILITY_CACHE_TTL = 0.12
function isShootButtonPressed()
    if IS_MOBILE then
        return MOBILE_TOUCH_STATE.touching
    end
    return UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)
end
function getScreenPointForAim()
    if IS_MOBILE then
...
-- 確保載入期間選單保持關閉，直到競爭者 UI 就緒
getgenv()._NOIR_UI_GATE = false -- false = 強制隱藏 NOIR UI
pcall(function()
    ...
    if Library.ScreenGui then
        Library.ScreenGui.Enabled = false -- 整包 UI 先關掉，避免搶在競爭者前面
        Library.ScreenGui.DisplayOrder = -999
    end
end)
-- 持續壓住 UI，直到 _NOIR_UI_GATE = true
pcall(function()
    local rs = game:GetService("RunService")
    local bind = "NoirUiGateHide"
    pcall(function() rs:UnbindFromRenderStep(bind) end)
    rs:BindToRenderStep(bind, Enum.RenderPriority.Last.Value, function()
        if getgenv()._NOIR_UI_GATE then return end
        pcall(function()
            if Library then
                Library.Toggled = false
                if Library.KeybindFrame then Library.KeybindFrame.Visible = false end
                if Library.SetWatermarkVisibility then
                    Library:SetWatermarkVisibility(false)
                end
            end
...
-- NOIR black & white theme
pcall(function()
    Library.AccentColor = Color3.fromRGB(235, 235, 235)
...
-- Progressive tab loading (one by one, reduces freeze)
NOIR_TAB_LOAD_STEP = 0
function noirTabYield(label)
    NOIR_TAB_LOAD_STEP = (NOIR_TAB_LOAD_STEP or 0) + 1
    -- progressive load silent (avoid notify spam)
    task.wait(0.03)
end
...
Tabs = {
	...
	['UI Settings'] = Window:AddTab('設定'),
}
qwe = nil; qwe=hookfunction(getrenv().setmetatable,newcclosure(function(Table,Metatable)
...
function instanceShowLoadingNotification()
    return function() end
end
getgenv().InstanceModuleCache = getgenv().InstanceModuleCache or {}
function instanceSafeRequire(moduleRef, timeoutSec)
    ...
    local deadline = os.clock() + (tonumber(timeoutSec) or 1.5)
    local lastErr
    while os.clock() < deadline do
...
function instancePreloadCoreModules()
...
function instanceHideErrorPromptsStep()
    local coreGui = game:GetService("CoreGui")
...
function instanceRunLoadingBootstrap()
    ...
    local hideConn = runService.RenderStepped:Connect(function()
        if hideErrors then pcall(instanceHideErrorPromptsStep) end
    end)
    -- short, non-blocking preload (modules load lazily later)
    task.spawn(function()
        task.wait(0.3)
        pcall(function()
            -- lighter preload: only try briefly
            local rs = game:GetService("ReplicatedStorage")
            local mods = rs:FindFirstChild("Modules")
            if mods then
                pcall(function()
                    local el = mods:FindFirstChild("EnumLibrary")
                    if el then instanceSafeRequire(el, 2) end
                end)
                pcall(function()
                    local ut = mods:FindFirstChild("Utility")
                    if ut then instanceSafeRequire(ut, 2) end
                end)
            end
        end)
        hideErrors = false
        if hideConn then hideConn:Disconnect() hideConn = nil end
        if dismissLoading then pcall(dismissLoading) end
    end)
end
RunService = game:GetService("RunService")
pcall(instanceRunLoadingBootstrap)
UserInputServiceMenu = game:GetService("UserInputService")
pcall(function()
    if setfpscap then pcall(setfpscap, 240) end
end)
function instancePlainReplace(str, find, replace)
    local startPos = 1
    while true do
...
function applyInstanceAccentTheme()
...
INSTANCE_MENU_DISPLAY_ORDER = 2147483646
INSTANCE_COSMETIC_DISPLAY_ORDER = 2147483647
INSTANCE_KEYBIND_LIST_ORDER = 80
INSTANCE_GAMEPLAY_OVERLAY_ORDER = 10
function applyInstanceUiLayering()
    pcall(function()
        if Library and Library.ScreenGui then
...
function ensureInstanceMenuCursor()
...
function shouldSuppressGameplayOverlays()
...
MENU_CURSOR_HZ = 60
MENU_CURSOR_BIND = "InstanceMenuCursorDraw"
menuCursorAccum = 0
function drawInstanceMenuCursor()
    local mc = getgenv().InstanceMenuCursor
    if not mc or not mc.initialized then return end
    local open = isInstanceMenuOpen()
    local useCustom = Library and Library.ShowCustomCursor
    if not open or not useCustom then
        if mc.cursor then mc.cursor.Visible = false end
        if mc.outline then mc.outline.Visible = false end
        mc.wasOpen = open
        return
    end
    -- custom triangle follows real mouse; hide OS cursor
    pcall(function()
        UserInputServiceMenu.MouseIconEnabled = false
    end)
    local m = UserInputServiceMenu:GetMouseLocation()
    local size = 12
    if mc.cursor then
        mc.cursor.PointA = Vector2.new(m.X, m.Y)
        mc.cursor.PointB = Vector2.new(m.X + size, m.Y + size * 0.85)
        mc.cursor.PointC = Vector2.new(m.X + size * 0.35, m.Y + size)
        mc.cursor.Color = Color3.fromRGB(255, 255, 255)
        mc.cursor.Visible = true
    end
    if mc.outline then
        mc.outline.PointA = Vector2.new(m.X, m.Y)
        mc.outline.PointB = Vector2.new(m.X + size, m.Y + size * 0.85)
        mc.outline.PointC = Vector2.new(m.X + size * 0.35, m.Y + size)
...
-- Mouse unlock (custom menu style): force free while open; return to game when closed + hide cursor
NOIR_MOUSE_BIND = "NoirMenuMouseUnlock"
NOIR_MENU_WAS_OPEN = false
function noirApplyMenuMouse()
    if IS_MOBILE then return end
    local uis = game:GetService("UserInputService")
    local open = false
    pcall(function()
        if isInstanceMenuOpen then open = isInstanceMenuOpen() end
        if not open and Window and Window.Holder then open = Window.Holder.Visible == true end
    end)
    if open then
        NOIR_MENU_WAS_OPEN = true
        -- free mouse for UI
        pcall(function()
            uis.MouseBehavior = Enum.MouseBehavior.Default
            if Library and Library.ShowCustomCursor then
                uis.MouseIconEnabled = false -- hide system cursor; custom draws
            else
                uis.MouseIconEnabled = true
            end
        end)
    else
        if NOIR_MENU_WAS_OPEN then
            NOIR_MENU_WAS_OPEN = false
            -- stop forcing unlock; hide custom/system cursor and return control to game
            pcall(function()
                uis.MouseIconEnabled = false
            end)
            -- hide drawing cursor overlays
            pcall(function()
                local mc = getgenv().InstanceMenuCursor
                if mc then
                    if mc.cursor then mc.cursor.Visible = false end
            ...
            -- do NOT force LockCenter here (third-person safe); game scripts re-lock if needed
        end
    end
end
pcall(function()
    RunService:UnbindFromRenderStep(NOIR_MOUSE_BIND)
end)
pcall(function()
    RunService:BindToRenderStep(NOIR_MOUSE_BIND, Enum.RenderPriority.Last.Value + 20, function()
        noirApplyMenuMouse()
    end)
end)

getgenv().InstanceLanguage = getgenv().InstanceLanguage or "chinese"
InstanceWeaponTranslations = {
    korean = {
...
InstanceLocaleTable = {
    english = {
        ...
        ready = "NOIR ready · RightShift to open menu",
        lang_changed = "Language: English",
        hema_combat = "HEMA Silent/Rage active",
        config_saved = "Config saved",
        config_loaded = "Config loaded",
        silent_aim = "Silent Aim", aimbot = "Aimbot", ragebot = "Ragebot",
        wall_check = "Wall Check", auto_shoot = "Auto Shoot",
    },
    korean = {
    ...
    chinese = {
        studs = "%.0f 距離", studs_short = "%d 距離", select_weapon = "請先選擇武器",
        apply = "套用", apply_all = "套用至全部", language = "語言",
        cosmetic_title = "外觀更換", weapons = "武器", cosmetics = "外觀",
        filter_weapons = "搜尋武器...", filter_cosmetics = "搜尋外觀...",
        skin = "皮膚", wrap = "包裝", rclick_hint = "右鍵武器：移除皮膚",
        unlock_all = "解鎖全部", inverted = "反轉", favorited = "已收藏",
        ready = "NOIR 已就緒 · 按 RightShift 開選單",
        lang_changed = "語言：中文",
        hema_combat = "HEMA 靜默/狂暴 已啟用",
        config_saved = "設定檔已儲存",
        config_loaded = "設定檔已讀取",
        silent_aim = "靜默自瞄", aimbot = "自瞄", ragebot = "狂暴自瞄",
        wall_check = "牆壁檢查", auto_shoot = "自動射擊",
    },
}
...
NOIR_UI_I18N = {
    ["FOV 半徑"] = { zh = "FOV 半徑", en = "FOV Radius" },
    ["FOV 填充"] = { zh = "FOV 填充", en = "FOV Fill" },
    ["FOV 旋轉"] = { zh = "FOV 旋轉", en = "FOV Spin" },
    ["FPS 上限（0 = 關閉）"] = { zh = "FPS 上限（0 = 關閉）", en = "FPS Cap (0 = off)" },
    ["FPS 數值"] = { zh = "FPS 數值", en = "FPS Value" },
    ["Unload Menu"] = { zh = "Unload Menu", en = "Unload Menu" },
    ["Visualizer"] = { zh = "Visualizer", en = "Visualizer" },
    ["X 比例"] = { zh = "X 比例", en = "X Ratio" },
    ["Y 比例"] = { zh = "Y 比例", en = "Y Ratio" },
    ["glow"] = { zh = "glow", en = "Glow" },
    ["light emission"] = { zh = "light emission", en = "Light Emission" },
    ["opacity max"] = { zh = "opacity max", en = "Opacity Max" },
    ["opacity min"] = { zh = "opacity min", en = "Opacity Min" },
    ["particle size"] = { zh = "particle size", en = "Particle Size" },
    ["spread"] = { zh = "spread", en = "Spread" },
    ["上一個設定檔"] = { zh = "上一個設定檔", en = "Previous Config" },
    ["上方色偏"] = { zh = "上方色偏", en = "Top Color Shift" },
    ["上色"] = { zh = "上色", en = "Chams" },
    ["下一個設定檔"] = { zh = "下一個設定檔", en = "Next Config" },
    ["下方偏移"] = { zh = "下方偏移", en = "Under Offset" },
    ["下方色偏"] = { zh = "下方色偏", en = "Bottom Color Shift" },
    ["不使用下拉表 · 請用下方按鈕"] = { zh = "不使用下拉表 · 請用下方按鈕", en = "No dropdown · use buttons below" },
    ["世界"] = { zh = "世界", en = "World" },
    ["主動畫 ID"] = { zh = "主動畫 ID", en = "Primary Anim ID" },
    ["主預設"] = { zh = "主預設", en = "Main Preset" },
    ["亮度"] = { zh = "亮度", en = "Brightness" },
    ["亮度閾值"] = { zh = "亮度閾值", en = "Brightness Threshold" },
    ["伺服器"] = { zh = "伺服器", en = "Server" },
    ["伺服器端"] = { zh = "伺服器端", en = "Server-side" },
    ["位置"] = { zh = "位置", en = "Position" },
    ["使用者 ID"] = { zh = "使用者 ID", en = "User ID" },
    ["偏移"] = { zh = "偏移", en = "Offset" },
    ["偏航角"] = { zh = "偏航角", en = "Yaw" },
    ["停用元素"] = { zh = "停用元素", en = "Disable Elements" },
    ["停用動畫"] = { zh = "停用動畫", en = "Disable Animations" },
    ["側向偏移"] = { zh = "側向偏移", en = "Side Offset" },
    ["偽裝"] = { zh = "偽裝", en = "Spoof" },
    ["僅可見"] = { zh = "僅可見", en = "Visible Only" },
    ["儲存設定檔"] = { zh = "儲存設定檔", en = "Save Config" },
    ["光暈大小"] = { zh = "光暈大小", en = "Bloom Size" },
    ["全域陰影"] = { zh = "全域陰影", en = "Global Shadows" },
    ["刪除選定設定檔"] = { zh = "刪除選定設定檔", en = "Delete Config" },
    ["前方偏移"] = { zh = "前方偏移", en = "Front Offset" },
    ["動畫"] = { zh = "動畫", en = "Animation" },
    ["動畫填充"] = { zh = "動畫填充", en = "Anim Fill" },
    ["卸載"] = { zh = "卸載", en = "Unload" },
    ["反次元地雷"] = { zh = "反次元地雷", en = "Anti Tripmine" },
    ["反武士刀"] = { zh = "反武士刀", en = "Anti Katana" },
    ["反閃光彈"] = { zh = "反閃光彈", en = "Anti Flashbang" },
    ["取消自動載入"] = { zh = "取消自動載入", en = "Clear Autoload" },
    ["名稱"] = { zh = "名稱", en = "Name" },
    ["名稱大小"] = { zh = "名稱大小", en = "Name Size" },
    ["命中率"] = { zh = "命中率", en = "Hit Chance" },
    ["命中部位"] = { zh = "命中部位", en = "Hit Part" },
    ["啟用"] = { zh = "啟用", en = "Enabled" },
    ["啟用弱點"] = { zh = "啟用弱點", en = "Enable Weakspot" },
    ["固定 X"] = { zh = "固定 X", en = "Lock X" },
    ["固定 Y"] = { zh = "固定 Y", en = "Lock Y" },
    ["圖片"] = { zh = "圖片", en = "Image" },
    ["地下"] = { zh = "地下", en = "Underground" },
    ["地理緯度"] = { zh = "地理緯度", en = "Latitude" },
    ["垂直偏移"] = { zh = "垂直偏移", en = "Vertical Offset" },
    ["堆疊間距"] = { zh = "堆疊間距", en = "Stack Spacing" },
    ["填充旋轉"] = { zh = "填充旋轉", en = "Fill Spin" },
    ["填充透明度"] = { zh = "填充透明度", en = "Fill Transparency" },
    ["填充速度"] = { zh = "填充速度", en = "Fill Speed" },
    ["大小"] = { zh = "大小", en = "Size" },
    ["天空盒"] = { zh = "天空盒", en = "Skybox" },
    ["太陽光色"] = { zh = "太陽光色", en = "Sun Color" },
    ["存活時間"] = { zh = "存活時間", en = "Lifetime" },
    ["室外環境光"] = { zh = "室外環境光", en = "Outdoor Ambient" },
    ["密度"] = { zh = "密度", en = "Density" },
    ["射擊次數"] = { zh = "射擊次數", en = "Shoot Attempts" },
    ["對比"] = { zh = "對比", en = "Contrast" },
    ["小時"] = { zh = "小時", en = "Hour" },
    ["工具"] = { zh = "工具", en = "Tool" },
    ["工具大小"] = { zh = "工具大小", en = "Tool Size" },
    ["工具類"] = { zh = "工具類", en = "Tool Class" },
    ["已驗證"] = { zh = "已驗證", en = "Verified" },
    ["平滑度"] = { zh = "平滑度", en = "Smoothness" },
    ["平滑材質"] = { zh = "平滑材質", en = "Smooth Materials" },
    ["延遲數值"] = { zh = "延遲數值", en = "Ping Value" },
    ["彈藥"] = { zh = "彈藥", en = "Ammo" },
    ["循環"] = { zh = "循環", en = "Loop" },
    ["快捷鍵"] = { zh = "快捷鍵", en = "Keybind" },
    ["快捷鍵列表"] = { zh = "快捷鍵列表", en = "Keybind List" },
    ["快速攻擊"] = { zh = "快速攻擊", en = "Rapid Attack" },
    ["戰鬥"] = { zh = "戰鬥", en = "Combat" },
    ["手臂上色"] = { zh = "手臂上色", en = "Arm Chams" },
    ["手臂描邊"] = { zh = "手臂描邊", en = "Arm Outline" },
    ["手臂材質"] = { zh = "手臂材質", en = "Arm Material" },
    ["手臂透明度"] = { zh = "手臂透明度", en = "Arm Transparency" },
    ["手臂高亮"] = { zh = "手臂高亮", en = "Arm Highlight" },
    ["手臂高亮填充透明度"] = { zh = "手臂高亮填充透明度", en = "Arm HL Fill Trans" },
    ["手臂高亮描邊透明度"] = { zh = "手臂高亮描邊透明度", en = "Arm HL Outline Trans" },
    ["手臂高亮材質"] = { zh = "手臂高亮材質", en = "Arm HL Material" },
    ["抖動動畫 ID"] = { zh = "抖動動畫 ID", en = "Jitter Anim ID" },
    ["抖動模式"] = { zh = "抖動模式", en = "Jitter Mode" },
    ["抖動間隔"] = { zh = "抖動間隔", en = "Jitter Interval" },
    ["抖動預設"] = { zh = "抖動預設", en = "Jitter Preset" },
    ["持續時間"] = { zh = "持續時間", en = "Duration" },
    ["排位"] = { zh = "排位", en = "Ranked" },
    ["排隊模式"] = { zh = "排隊模式", en = "Queue Mode" },
    ["描邊旋轉"] = { zh = "描邊旋轉", en = "Outline Spin" },
    ["描邊粗細"] = { zh = "描邊粗細", en = "Outline Thickness" },
    ["描邊透明度"] = { zh = "描邊透明度", en = "Outline Transparency" },
    ["插值速度"] = { zh = "插值速度", en = "Lerp Speed" },
    ["換彈時隱藏"] = { zh = "換彈時隱藏", en = "Hide On Reload" },
    ["搜尋武器 1"] = { zh = "搜尋武器 1", en = "Search Weapon 1" },
    ["搜尋武器 2"] = { zh = "搜尋武器 2", en = "Search Weapon 2" },
    ["搜尋音效"] = { zh = "搜尋音效", en = "Search Sound" },
    ["播放速度"] = { zh = "播放速度", en = "Playback Speed" },
    ["擊中通知"] = { zh = "擊中通知", en = "Hit Notifications" },
    ["攻擊"] = { zh = "攻擊", en = "Attack" },
    ["攻擊位置"] = { zh = "攻擊位置", en = "Attack Position" },
    ["文字大小"] = { zh = "文字大小", en = "Text Size" },
    ["方框"] = { zh = "方框", en = "Box" },
    ["方框填充"] = { zh = "方框填充", en = "Box Fill" },
    ["方框發光"] = { zh = "方框發光", en = "Box Glow" },
    ["旋轉"] = { zh = "旋轉", en = "Spin" },
    ["旋轉速度"] = { zh = "旋轉速度", en = "Spin Speed" },
    ["最多顯示"] = { zh = "最多顯示", en = "Max Display" },
    ["最大尺寸"] = { zh = "最大尺寸", en = "Max Size" },
    ["最大精準"] = { zh = "最大精準", en = "Max Accuracy" },
    ["最大角度"] = { zh = "最大角度", en = "Max Angle" },
    ["最大距離"] = { zh = "最大距離", en = "Max Range" },
    ["最大速度"] = { zh = "最大速度", en = "Max Speed" },
    ["最小尺寸"] = { zh = "最小尺寸", en = "Min Size" },
    ["最小角度"] = { zh = "最小角度", en = "Min Angle" },
    ["最小速度"] = { zh = "最小速度", en = "Min Speed" },
    ["材質"] = { zh = "材質", en = "Material" },
    ["槍材質"] = { zh = "槍材質", en = "Gun Material" },
    ["槍枝上色"] = { zh = "槍枝上色", en = "Gun Chams" },
    ["槍枝描邊"] = { zh = "槍枝描邊", en = "Gun Outline" },
    ["槍枝高亮"] = { zh = "槍枝高亮", en = "Gun Highlight" },
    ["槍透明度"] = { zh = "槍透明度", en = "Gun Transparency" },
    ["槍高亮材質"] = { zh = "槍高亮材質", en = "Gun HL Material" },
    ["模組偵測"] = { zh = "模組偵測", en = "Mod Detector" },
    ["樣式"] = { zh = "樣式", en = "Style" },
    ["武器"] = { zh = "武器", en = "Weapon" },
    ["武器 1"] = { zh = "武器 1", en = "Weapon 1" },
    ["武器 2"] = { zh = "武器 2", en = "Weapon 2" },
    ["浮水印"] = { zh = "浮水印", en = "Watermark" },
    ["淡出時間"] = { zh = "淡出時間", en = "Fade Out Time" },
    ["深色材質"] = { zh = "深色材質", en = "Dark Materials" },
    ["準心模式"] = { zh = "準心模式", en = "Crosshair Mode" },
    ["準心線"] = { zh = "準心線", en = "Crosshair Lines" },
    ["滑鏟加速"] = { zh = "滑鏟加速", en = "Slide Boost" },
    ["滑鏟加速數值"] = { zh = "滑鏟加速數值", en = "Slide Boost Value" },
    ["漂浮速度"] = { zh = "漂浮速度", en = "Float Speed" },
    ["漫反射"] = { zh = "漫反射", en = "Diffuse" },
    ["漫反射強度"] = { zh = "漫反射強度", en = "Diffuse Scale" },
    ["漸層速度"] = { zh = "漸層速度", en = "Gradient Speed" },
    ["無動畫"] = { zh = "無動畫", en = "No Animations" },
    ["無後座力"] = { zh = "無後座力", en = "No Recoil" },
    ["無散射"] = { zh = "無散射", en = "No Spread" },
    ["無槍口火光"] = { zh = "無槍口火光", en = "No Muzzle Flash" },
    ["牆壁檢查"] = { zh = "牆壁檢查", en = "Wall Check" },
    ["特效"] = { zh = "特效", en = "Effect" },
    ["狂暴狀態"] = { zh = "狂暴狀態", en = "Rage Status" },
    ["狂暴自瞄"] = { zh = "狂暴自瞄", en = "Ragebot" },
    ["環境光"] = { zh = "環境光", en = "Ambient" },
    ["環境音效"] = { zh = "環境音效", en = "Ambient Sound" },
    ["環繞半徑"] = { zh = "環繞半徑", en = "Orbit Radius" },
    ["環繞高度"] = { zh = "環繞高度", en = "Orbit Height" },
    ["生成率"] = { zh = "生成率", en = "Spawn Rate" },
    ["發光旋轉"] = { zh = "發光旋轉", en = "Glow Spin" },
    ["發光透明度"] = { zh = "發光透明度", en = "Glow Transparency" },
    ["目前選定: （尚未選擇）"] = { zh = "目前選定: （尚未選擇）", en = "Selected: (none)" },
    ["眩光"] = { zh = "眩光", en = "Glare" },
    ["瞄準曲線"] = { zh = "瞄準曲線", en = "Aim Curve" },
    ["穿牆"] = { zh = "穿牆", en = "Noclip" },
    ["等級"] = { zh = "等級", en = "Level" },
    ["背後射擊"] = { zh = "背後射擊", en = "Backshoot" },
    ["脈衝動畫速度"] = { zh = "脈衝動畫速度", en = "Pulse Speed" },
    ["自動射擊"] = { zh = "自動射擊", en = "Auto Shoot" },
    ["自動鎖定"] = { zh = "自動鎖定", en = "Auto Target" },
    ["自發光"] = { zh = "自發光", en = "Self Glow" },
    ["自瞄"] = { zh = "自瞄", en = "Aimbot" },
    ["自訂 X"] = { zh = "自訂 X", en = "Custom X" },
    ["自訂 Y"] = { zh = "自訂 Y", en = "Custom Y" },
    ["自訂名稱"] = { zh = "自訂名稱", en = "Custom Name" },
    ["自訂地區"] = { zh = "自訂地區", en = "Custom Region" },
    ["自訂時間"] = { zh = "自訂時間", en = "Custom Time" },
    ["自訂游標"] = { zh = "自訂游標", en = "Custom Cursor" },
    ["自訂緯度"] = { zh = "自訂緯度", en = "Custom Latitude" },
    ["自訂角度"] = { zh = "自訂角度", en = "Custom Angle" },
    ["自訂霧"] = { zh = "自訂霧", en = "Custom Fog" },
    ["虛空連射"] = { zh = "虛空連射", en = "Void Spam" },
    ["血條"] = { zh = "血條", en = "Health Bar" },
    ["裝置"] = { zh = "裝置", en = "Device" },
    ["視覺"] = { zh = "視覺", en = "Visuals" },
    ["角度"] = { zh = "角度", en = "Angle" },
    ["角色"] = { zh = "角色", en = "Character" },
    ["解鎖全部"] = { zh = "解鎖全部", en = "Unlock All" },
    ["設定"] = { zh = "設定", en = "Settings" },
    ["設定檔"] = { zh = "設定檔", en = "Configs" },
    ["設定檔名稱"] = { zh = "設定檔名稱", en = "Config Name" },
    ["設為自動載入"] = { zh = "設為自動載入", en = "Set Autoload" },
    ["語言"] = { zh = "語言", en = "Language" },
    ["讀取設定檔"] = { zh = "讀取設定檔", en = "Load Config" },
    ["距離"] = { zh = "距離", en = "Distance" },
    ["距離大小"] = { zh = "距離大小", en = "Distance Size" },
    ["跟隨槍口"] = { zh = "跟隨槍口", en = "Follow Muzzle" },
    ["軌跡"] = { zh = "軌跡", en = "Tracers" },
    ["軌跡描邊"] = { zh = "軌跡描邊", en = "Tracer Outline" },
    ["軌跡粗細"] = { zh = "軌跡粗細", en = "Tracer Thickness" },
    ["軌跡起點"] = { zh = "軌跡起點", en = "Tracer Origin" },
    ["退場動畫"] = { zh = "退場動畫", en = "Out Animation" },
    ["退場速度"] = { zh = "退場速度", en = "Out Speed" },
    ["透明度"] = { zh = "透明度", en = "Transparency" },
    ["透明材質"] = { zh = "透明材質", en = "Transparent Materials" },
    ["透視透明度"] = { zh = "透視透明度", en = "XRay Transparency" },
    ["速度"] = { zh = "速度", en = "Speed" },
    ["速度修改"] = { zh = "速度修改", en = "Speed Modifier" },
    ["速度數值"] = { zh = "速度數值", en = "Speed Value" },
    ["連勝"] = { zh = "連勝", en = "Win Streak" },
    ["連發"] = { zh = "連發", en = "Rapid Fire" },
    ["進場動畫"] = { zh = "進場動畫", en = "In Animation" },
    ["進場速度"] = { zh = "進場速度", en = "In Speed" },
    ["選單"] = { zh = "選單", en = "Menu" },
    ["選單快捷鍵"] = { zh = "選單快捷鍵", en = "Menu Keybind" },
    ["重新整理列表"] = { zh = "重新整理列表", en = "Refresh List" },
    ["重生保護"] = { zh = "重生保護", en = "Spawn Protect" },
    ["開啟快捷鍵選單"] = { zh = "開啟快捷鍵選單", en = "Show Keybind Menu" },
    ["關閉槍聲音效"] = { zh = "關閉槍聲音效", en = "Mute Gun Sounds" },
    ["陰影柔和"] = { zh = "陰影柔和", en = "Soft Shadows" },
    ["陰影柔和度"] = { zh = "陰影柔和度", en = "Shadow Softness" },
    ["隨機角度"] = { zh = "隨機角度", en = "Random Angle" },
    ["隱藏"] = { zh = "隱藏", en = "Hide" },
    ["隱藏手臂"] = { zh = "隱藏手臂", en = "Hide Arms" },
    ["雜項"] = { zh = "雜項", en = "Misc" },
    ["霧終點"] = { zh = "霧終點", en = "Fog End" },
    ["霧起點"] = { zh = "霧起點", en = "Fog Start" },
    ["霾"] = { zh = "霾", en = "Haze" },
    ["靜默自瞄"] = { zh = "靜默自瞄", en = "Silent Aim" },
    ["音效"] = { zh = "音效", en = "Sound" },
    ["音量"] = { zh = "音量", en = "Volume" },
    ["音高"] = { zh = "音高", en = "Pitch" },
    ["預判"] = { zh = "預判", en = "Prediction" },
    ["預判倍率"] = { zh = "預判倍率", en = "Prediction Mult" },
    ["顯示 FOV"] = { zh = "顯示 FOV", en = "Show FOV" },
    ["顯示彈藥列"] = { zh = "顯示彈藥列", en = "Show Ammo Bar" },
    ["飛行模式"] = { zh = "飛行模式", en = "Fly Mode" },
    ["飛行速度"] = { zh = "飛行速度", en = "Fly Speed" },
    ["飽和度"] = { zh = "飽和度", en = "Saturation" },
    ["骨架"] = { zh = "骨架", en = "Skeleton" },
    ["骨架粗細"] = { zh = "骨架粗細", en = "Skeleton Thickness" },
    ["高光"] = { zh = "高光", en = "Specular" },
    ["高光強度"] = { zh = "高光強度", en = "Specular Scale" },
    ["高級"] = { zh = "高級", en = "Advanced" },
}
NOIR_UI_I18N_BY_EN = {}
for _, pack in pairs(NOIR_UI_I18N) do
    if pack.en then NOIR_UI_I18N_BY_EN[pack.en] = pack end
    if pack.zh then NOIR_UI_I18N[pack.zh] = pack end
end

function noirUiTranslate(text)
    if type(text) ~= "string" or text == "" then return text end
    local lang = getgenv().InstanceLanguage or "chinese"
    local pack = NOIR_UI_I18N[text] or NOIR_UI_I18N_BY_EN[text]
    if not pack then return text end
    if lang == "english" or lang == "en" then
        return pack.en or text
    end
    return pack.zh or text
end
getgenv().NoirUiTranslate = noirUiTranslate

function noirTrySetControlText(ctrl, newText)
    if type(ctrl) ~= "table" or not newText then return end
    pcall(function()
        ctrl.Text = newText
        if type(ctrl.SetText) == "function" then
            ctrl:SetText(newText)
        end
        -- Linoria often keeps a label under Container / TextLabel
        if ctrl.Label and type(ctrl.Label) == "userdata" then
            if ctrl.Label:IsA("TextLabel") or ctrl.Label:IsA("TextButton") then
                ctrl.Label.Text = newText
            end
        end
        if ctrl.TextLabel and typeof(ctrl.TextLabel) == "Instance" then
            ctrl.TextLabel.Text = newText
        end
    end)
end

function noirRefreshLocaleUI()
    local lang = getgenv().InstanceLanguage or "chinese"
    -- Toggles
    pcall(function()
        if Toggles then
            for _, t in pairs(Toggles) do
                if type(t) == "table" and type(t.Text) == "string" then
                    local nt = noirUiTranslate(t.Text)
                    if nt ~= t.Text then
                        noirTrySetControlText(t, nt)
                    else
                        -- try translate via map even if already zh/en
                        local pack = NOIR_UI_I18N[t.Text] or NOIR_UI_I18N_BY_EN[t.Text]
                        if pack then
                            local want = (lang == "english" or lang == "en") and pack.en or pack.zh
                            if want and want ~= t.Text then
                                noirTrySetControlText(t, want)
                            end
                        end
                    end
                end
    ...
    -- Options (sliders/dropdowns/inputs)
    pcall(function()
        if Options then
            for _, o in pairs(Options) do
                if type(o) == "table" and type(o.Text) == "string" then
                    local pack = NOIR_UI_I18N[o.Text] or NOIR_UI_I18N_BY_EN[o.Text]
                    if pack then
                        local want = (lang == "english" or lang == "en") and pack.en or pack.zh
                        if want then
                            noirTrySetControlText(o, want)
                        end
                    end
                end
            end
    ...
    -- Walk visible ScreenGui text labels that match known strings (best-effort)
    pcall(function()
        if not Library or not Library.ScreenGui then return end
        for _, inst in ipairs(Library.ScreenGui:GetDescendants()) do
            if inst:IsA("TextLabel") or inst:IsA("TextButton") then
                local pack = NOIR_UI_I18N[inst.Text] or NOIR_UI_I18N_BY_EN[inst.Text]
                if pack then
                    local want = (lang == "english" or lang == "en") and pack.en or pack.zh
                    if want and want ~= inst.Text then
                        inst.Text = want
                    end
                end
            end
    ...
    pcall(function()
        if NoirNotify then
            if lang == "english" or lang == "en" then
                NoirNotify("Language: English (menu labels refreshed)", 3)
            else
                NoirNotify("語言：中文（選單文字已更新）", 3)
            end
        end
    end)
end
getgenv().InstanceRefreshLocaleUI = noirRefreshLocaleUI

InstanceFontHttp = game:GetService("HttpService")
InstanceFontSources = {
...
function loadInstanceLanguageFont(lang)
...
function alignInstanceMenuTabs()
...
UserInputService = game:GetService("UserInputService")
-- Fast module bind (no multi-second blocking wait)
Utility = {
    EncodeCFrame = function(_, cf) return cf end,
}
EnumLibrary = {
    ToEnum = function(_, s) return s end,
}
pcall(function()
    local mods = ReplicatedStorage:FindFirstChild("Modules")
    if not mods then return end
    local u = mods:FindFirstChild("Utility")
    local e = mods:FindFirstChild("EnumLibrary")
    if u then
...
task.spawn(function()
    for _ = 1, 15 do
        ...
        pcall(function()
            local mods = ReplicatedStorage:FindFirstChild("Modules")
            if not mods then return end
            if not rawget(getfenv(), "_noirUtilBound") then
                local u = mods:FindFirstChild("Utility")
                if u then
                    local ok, res = pcall(require, u)
                    if ok then Utility = res; getgenv()._noirUtilBound = true end
                end
            end
            local e = mods:FindFirstChild("EnumLibrary")
            if e then
        ...
        if getgenv()._noirUtilBound then break end
    end
end)

-- ===== Lunara combat module bind (silent + rage) =====
function noirEnsureCombatModules()
    local rs = game:GetService("ReplicatedStorage")
    local lp = game:GetService("Players").LocalPlayer
    pcall(function()
        local mods = rs:FindFirstChild("Modules")
        if mods then
            ...
            if u then
                ...
                if ok and res then
                    Utility = res
                    getgenv()._noirUtilBound = true
                    if modules then modules.utility = res end
                end
            end
            local e = mods:FindFirstChild("EnumLibrary")
            if e then
                ...
                if ok and res then
                    EnumLibrary = res
                    if modules then modules.enums = res end
                end
            end
        end
    end)
    pcall(function()
        ...
        if fm and fm:IsA("ModuleScript") then
            local ok, fc = pcall(require, fm)
            if ok and fc then
                localFighter = fc.LocalFighter or fc.localFighter or localFighter
                if modules then modules.fighter = fc end
                if localfighter == nil and fc.LocalFighter then
                    localfighter = fc.LocalFighter
                end
            end
        end
        local cm = ctrl and ctrl:FindFirstChild("CameraController")
        if cm and cm:IsA("ModuleScript") then
            local ok, res = pcall(require, cm)
            if ok and res then
                camController = res
                if modules then modules.camcontrol = res end
            end
        end
    end)
    return Utility, EnumLibrary, localFighter
end
...
function applyAspectToViewport(screenVec, cam)
...
function screenAnchor(xNorm, yNorm, cam)
    cam = cam or workspace.CurrentCamera
    if not cam then
        return Vector2.zero
    end
...
function screenCenter(cam)
    return screenAnchor(0.5, 0.5, cam)
end
function getUnstretchedCameraCFrame(cam)
    cam = cam or workspace.CurrentCamera
    if not cam then
        return nil
    end
...
do
noirTabYield("戰鬥 · 靜默/自瞄")
RightGroup = Tabs.Combat:AddLeftTabbox()
silenttab = RightGroup:AddTab('靜默')
silentcustomization = RightGroup:AddTab('Customization')
AimbotGroup = Tabs.Combat:AddRightTabbox()
aimbotTab = AimbotGroup:AddTab('自瞄')
aimbotCustomTab = AimbotGroup:AddTab('Customization')
textureAssets, soundsassets, HPlist, restricteditems = nil
do
    restricteditems = {
...
detectkatana()
function katanadeflect(player)
    return katanausers[player] == true
end
function weaponstricted(weaponName)
...
silentAim = { enabled=false, hitPart="Head", fovRadius=120, autoShoot=false, followMuzzle=false, hitChance=100, wallCheck=false, maxRange=800 }
aimbot = {
    enabled = false,
    masterEnabled = false,
    keyMode = "toggle",
...
function makebullettracer(pos3, endPos, isHit)
...
function findShotMuzzlePosition()
...
function shootSoundsActive()
    return _G.Features.DisableGunSounds == true
end
function restoreGunSoundVolumes()
...
function bulletFeedbackActive()
    return bulletTracer.enabled
end
function getFighterController()
...
function isProjectilePartName(name)
    if not name then
        return false
    end
...
function isLikelyLocalProjectile(inst)
    if not inst then
        return false
    end
...
function isRivalsGunSound(sound)
...
function silenceRivalsGunSound(sound)
...
function bindGunSoundMuteGuard(sound)
...
function registerLocalShot(bypassCooldown)
    local cw = curweap2()
...
function handleLocalShot(bypassCooldown)
...
function startAmmoBulletDetection()
...
function bindProjectileImpactDetection(projectile)
...
function startProjectileBulletDetection()
...
function bindCombatShotSounds(root)
    if not root then
        return
    end
    for _, d in ipairs(root:GetDescendants()) do
...
RunService.RenderStepped:Connect(function()
    if not silentFOVContainer or not aimbotFOVContainer then return end
...
function hitpartfromname(target, partName)
...
function silentHasLOS(fromPos, toPos, targetChar)
    local params = RaycastParams.new()
...
-- Lunara closestinfov (+ optional wall / range / team fix)
function closestinfov(radius, center)
    local closest, closestDist = nil, math.huge
    local cam = Camera or workspace.CurrentCamera
    if not cam or not center then return nil end
    local myRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    local maxRange = tonumber(silentAim.maxRange) or 2000
    local playersList = Players:GetPlayers()
    for i = 1, #playersList do
        local plr = playersList[i]
        if plr ~= LocalPlayer then
            local tidA = LocalPlayer:GetAttribute("TeamID")
            local tidB = plr:GetAttribute("TeamID")
            local isMate = false
            if tidA ~= nil and tidB ~= nil and tidA == tidB then
                isMate = true
            elseif (tidA == nil or tidB == nil) and LocalPlayer.Team and plr.Team and LocalPlayer.Team == plr.Team then
                isMate = true
            end
            if not isMate then
                ...
                if char then
                    ...
                    if hum and hum.Health > 0 and not char:FindFirstChildOfClass("ForceField") then
                        ...
                        if root then
                            ...
                            if (not myRoot) or worldDist <= maxRange then
                                ...
                                if onScreen then
                                    ...
                                    if dist <= radius and dist < closestDist then
                                        local okTarget = true
                                        if silentAim.wallCheck and myRoot then
                                            okTarget = silentHasLOS(myRoot.Position, root.Position, char)
                                        end
                                        if okTarget then
                                            closest = char
                                            closestDist = dist
...
function closestplayerinfov(radius)
    local c = closestinfov(radius, silentfovcenter())
    curtarget = c
    return c
end
localFighter = localFighter
lastFireTime = lastFireTime or 0
fireCooldown = fireCooldown or 0.05
-- Lunara firesilent (module bind restored each shot)
function firesilent()
    ...
    if not root then return end
    if silentAim.wallCheck and not silentHasLOS(root.Position, part.Position, closest) then return end
    noirEnsureCombatModules()
    local equipped = localFighter and localFighter.EquippedItem
    if not equipped then return end
    local objId = nil
    pcall(function() objId = equipped:Get("ObjectID") end)
    ...
    getgenv().InstanceCombatLastShotAt = now
    if tp then
        local hum = closest:FindFirstChildOfClass("Humanoid")
        if hum and recordLocalHitTarget then
            pcall(recordLocalHitTarget, tp, hum, silentAim.hitPart or part.Name)
        end
        if ragePerf and ragePerf.hitPartByPlayer then
            ragePerf.hitPartByPlayer[tp] = silentAim.hitPart or part.Name
        end
        if ragePerf and ragePerf.lastHitAtByPlayer then
            ragePerf.lastHitAtByPlayer[tp] = now
        end
    end
end
...
-- Lunara-style combat controller bind (background, keeps retrying)
camController = camController
task.spawn(function()
    for _ = 1, 40 do
        pcall(noirEnsureCombatModules)
        if localFighter and Utility and getgenv()._noirUtilBound then
            print("[NOIR] combat modules ready · Fighter OK · Utility OK")
            break
        end
        task.wait(0.5)
    end
    if not localFighter then
        warn("[NOIR] FighterController not bound yet — silent/rage will retry on each shot")
    end
end)
...
function isTargetVisible(targetPart, myRoot)
...
function getAimbotLerpAlpha(dt)
...
AIMBOT_RENDER_BIND = "InstanceAimbotUpdate"
aimbotConnection = nil
aimbotUsingBind = false
function stepAimbot(dt)
    dt = dt or (1 / 240)
    -- Master toggle must be on; only aim while holding right mouse button
    if not aimbot.masterEnabled then
        clearAimbotLock()
        return
    end
...
function startBackshootMonitor()
...
silenttab:AddToggle("SilentAutoShoot", {
    Text = "自動射擊",
    Default = false,
    Callback = function(val) silentAim.autoShoot = val == true end
})
silenttab:AddToggle("SilentWallCheck", {
    Text = "牆壁檢查",
    Default = false,
    Callback = function(val) silentAim.wallCheck = val == true end
})
silenttab:AddSlider("SilentMaxRange", {
    Text = "最大距離",
    Default = 800,
    Min = 50,
    Max = 2000,
    Rounding = 0,
    Compact = true,
    Callback = function(val) silentAim.maxRange = val end
})
...
-- Aim only while holding RMB (handled inside stepAimbot)
...
aimbotCustomTab:AddSlider("AimbotFOVOutlineThickness", {
    Text = "描邊粗細", Default=1.5, Min=0.5, Max=5, Rounding=1, Compact=true,
...
aimbotCustomTab:AddSlider("AimbotFOVOutlineTransparency", {
    Text = "描邊透明度", Default=0, Min=0, Max=1, Rounding=2, Compact=true,
...
aimbotCustomTab:AddSlider("AimbotFOVOutlineRotation", {
    Text = "描邊旋轉", Default=0, Min=0, Max=360, Rounding=0, Compact=true,
...
do
noirTabYield("戰鬥 · 槍械")
LeftGroup = Tabs.Combat:AddLeftGroupbox("槍械")
Players = game:GetService("Players")
LocalPlayer = Players.LocalPlayer
...
hitEffectsTab = HitGroup:AddTab('擊中特效')
hitEffectEnabled = false
hitEffectColor = Color3.fromRGB(159, 133, 195)
hitEffectStyle = {Particles = true}
hitEffectFloatSpeed = 7
hitEffectAliveTime = 2.8
hitEffectAliveScale = 1
hitEffectsToggle = hitEffectsTab:AddToggle("HitEffects", {
    Text = "啟用",
    Default = false,
    Callback = function(val)
...
hitEffectDepBox = hitEffectsTab:AddDependencyBox()
...
function getHitEffectPart(adornee)
...
function blendHitColor(base, amount)
...
function darkenHitColor(base, amount)
...
function getHitEffectAliveScale()
    return hitEffectAliveScale
end
function scaleHitDuration(seconds)
    return seconds * getHitEffectAliveScale()
end
function each_style(callback)
...
function spawnHitRing(targetPart, color, config)
    config = config or {}
...
function spawnHitFlash(targetPart, color, brightness, range, duration)
...
function spawnHitEmitter(targetPart, config)
...
function particledesign(adornee, color)
...
function getActiveFortniteBillboards()
    local count = 0
...
function fortniteffect(adornee, color, damageText)
    color = color or hitEffectColor
...
function shockwaveEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function lightningEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function bloodEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function fireEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function iceEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function heartsEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function starsEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function confettiEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function rippleEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function sparksEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function neonPulseEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function voidRiftEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function plasmaEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
function glitchEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
    for i = 1, 6 do
        spawnHitEmitter(targetPart, {
...
function cosmicDustEffect(adornee, color)
    local targetPart = getHitEffectPart(adornee)
    if not targetPart then return end
...
do
hitSoundsTab = HitGroup:AddTab('擊中音效')
...
do
noirTabYield("世界 · 第一人稱")
ViewModelTabbox = Tabs.World:AddRightTabbox()
vmTab = ViewModelTabbox:AddTab("第一人稱模型")
viewportTab = ViewModelTabbox:AddTab("視角模型")
...
offsetY = 0
offsetZ = 0
function saveorigprops(descendant)
...
function restoreorigprops(descendant)
...
function forceApplyChamPart(part, fillColor, material, transparency, aggressive)
    saveorigprops(part)
    pcall(function()
...
function forceApplyArmChamPart(part, fillColor, material, transparency)
...
function gradcolor(c1, c2, c3, pos, t)
    local cp = (pos + t) % 1
    if cp < 0.33 then
...
function getModelYBounds(model)
...
function getFilteredModelYBounds(model, partFilter)
    local minY, maxY = math.huge, -math.huge
    for _, d in ipairs(model:GetDescendants()) do
...
function verticalChamColor(part, model, topColor, bottomColor, yBounds)
...
function animatedHighlightColor(topColor, bottomColor, phase)
    local t = phase * 2
    if t > 1 then
        t = 2 - t
    end
...
function shouldOutlinePart(part)
...
function clearChamOutlineGlow(part)
...
function clearAllChamOutlineGlows()
    local tracked = {}
...
function clearChamOutline(part)
...
function buildChamOutlineGlow(anchorPart, sourcePart, cloneTable, name, scale, transparency, padding)
...
function syncChamOutlineGlow(part, color, strength)
...
function syncChamOutline(part, color)
    if not part or not part.Parent then
        return
    end
...
function purgeStaleChamOutlines(activeParts)
    for part in pairs(chamOutlineHighlights) do
...
function viewModelEffectsActive()
...
function clearVmPartHighlight(part)
...
function clearAllVmPartHighlights()
    for part in pairs(vmPartHighlights) do
        clearVmPartHighlight(part)
    end
    vmPartHighlights = {}
end
function syncVmPartHighlight(part, fillColor, outlineColor, fillTrans, outlineTrans)
    if not part or not part.Parent then
        clearVmPartHighlight(part)
        return
    end
...
function purgeStaleVmPartHighlights(activeParts)
    for part in pairs(vmPartHighlights) do
        if not activeParts[part] then
            clearVmPartHighlight(part)
        end
...
function isArmNamedPart(partName)
...
function hasArmHierarchy(part, model)
...
function isArmChamPart(part, model)
    if not part or not part:IsA("BasePart") then
        return false
    end
...
function isGunChamPart(part, model)
    if not part or not part:IsA("BasePart") then
        return false
    end
    if OUTLINE_EXCLUDED_PARTS[part.Name] or part.Transparency >= 0.99 then
...
applyChams = nil
chamLoop = nil
CHAM_RENDER_BIND = "InstanceViewModelChams"
CHAM_TARGET_HZ = 60
chamAccum = 0
chamPartCache = nil
chamCacheDirty = true
...
chamFpHooked = false
function markChamCacheDirty()
    chamCacheDirty = true
end
function fullChamCleanup()
...
function hookChamFirstPerson(fp)
...
function rebuildChamPartCache(fp)
    chamPartCache = {}
    for _, model in ipairs(fp:GetChildren()) do
        if model:IsA("Model") then
...
function updViewModelEffectLoop()
    markChamCacheDirty()
...
function getViewModelTrackName(track)
...
function shouldBlockViewModelTrack(track)
...
function stopBlockedViewModelTrack(track)
...
function bindViewModelAnimator(animator)
...
function scanViewModelAnimators(root)
    if not root then return end
...
function hookViewModelAnimations()
    local vm = Workspace:FindFirstChild("ViewModels")
    if not vm then return end
    local fp = vm:FindFirstChild("FirstPerson")
    if fp then
...
do
noirTabYield("世界 · 軌跡")
TracersGroup = Tabs.World:AddLeftGroupbox("子彈軌跡")
tracerEnabled = true
...
function getTracerDrawEnd(tr, age)
...
function makeLineTracer(pos3, endPos)
...
function makeBeamTracer(pos3, endPos)
...
function maketracer(pos3, endPos)
...
function getEspPlayerWeapon(player)
...
pcall(function()
    modules.enums = instanceSafeRequire(replicatedstorage.Modules.EnumLibrary)
end)
pcall(function()
    modules.fighter = instanceSafeRequire(player.PlayerScripts.Controllers.FighterController)
end)
pcall(function()
    modules.camcontrol = instanceSafeRequire(player.PlayerScripts.Controllers.CameraController)
end)
pcall(function()
    modules.utility = instanceSafeRequire(replicatedstorage.Modules.Utility)
end)
pcall(noirEnsureCombatModules)
if not modules.utility then modules.utility = Utility end
if not modules.enums then modules.enums = EnumLibrary end
if modules.fighter and modules.fighter.LocalFighter then
    localfighter = modules.fighter.LocalFighter
    localFighter = modules.fighter.LocalFighter
end
MATCH_ID_ATTRS = {"MatchId", "DuelId", "RoundId", "GameId", "MatchUUID", "ArenaId", "InstanceId"}
...
_instanceLobbyCache = { t = 0, v = true }
function lobbyCache()
    local now = os.clock()
...
function tickAmmo()
    ...
    local current, maxAmmo, melee = 0, 0, false
    pcall(function()
    ...
    if melee then
        ...
        config.state.outofammo = current <= 0
    end
...
function isteammate(targetplayer)
    if not targetplayer then return false end
    local a = player:GetAttribute("TeamID")
    local b = targetplayer:GetAttribute("TeamID")
    -- both nil would be true in Lua; only treat as teammate when both exist and match
    if a == nil or b == nil then
        local ta, tb = player.Team, targetplayer.Team
...
function hitpartfromname(character, partname)
...
function predict(targetpart, origin)
...
function rndSkip(mi, ma, dmi, dma)
...
function autoshoot()
    ...
    eqSlot()
    noirEnsureCombatModules()
    local lf = (modules and modules.fighter and modules.fighter.LocalFighter) or localFighter or localfighter
    if modules and modules.fighter and modules.fighter.LocalFighter then localFighter = modules.fighter.LocalFighter; localfighter = modules.fighter.LocalFighter end
    ...
    noirEnsureCombatModules()
    local util = (modules and modules.utility) or Utility
    local enums = (modules and modules.enums) or EnumLibrary
    if not util or not util.EncodeCFrame then return end
    if not enums or not enums.ToEnum then return end
    local data = {
        [utf8.char(1)] = {
            [utf8.char(0)] = util:EncodeCFrame(CFrame.new(shootPos, targetPos)),
            [utf8.char(1)] = util:EncodeCFrame(CFrame.new(shootPos, targetPos)),
            [utf8.char(2)] = hitPart,
    ...
    local objId = nil
    pcall(function() objId = equipped and equipped:Get("ObjectID") end)
    if equipped and objId then
...
pcall(function()
    noirEnsureCombatModules()
    if modules and modules.camcontrol and type(modules.camcontrol.Update) == "function" then
...
onImmune = {}
onVulnerable = {}
function bindimmune(callback)
    table.insert(onImmune, callback)
end
function bindvulnerable(callback)
...
function updatesling()
    ...
    if slingon then
        ...
        local undergroundOn = Toggles and Toggles.AntiAimUnderground and Toggles.AntiAimUnderground.Value
        if not undergroundOn then
            if getgenv().InstanceSetUnderground then
...
runsvc.Heartbeat:Connect(function()
    ...
    updatesling()
    tickAmmo()
    if (config.target.auto or config.target.rageMasterOn) and not config.target.manualkey then
        local current, _, melee = getammo()
...
rageui.ragebottab:AddToggle("TargetOn", {
    ...
    Callback = function(val)
        ...
        -- allow continuous retarget while master is on
        config.target.manualkey = false
        local target = nearest()
        if target then
            settarget(target)
        end
        NoirNotify("狂暴已開啟 · 請裝備武器", 2)
    end
...
attackUnderVisGate = { Type = "Toggle", Value = false }
attackCustomVisGate = { Type = "Toggle", Value = false }
function syncAtkDeps()
    local mode = config.target.attackPosition or "default"
...
attackPosDropdown = rageui.ragebottab:AddDropdown("AutoTargetAttackPos", {
    Text = "攻擊位置",
    Default = "random",
    Values = { "random", "behind", "under", "above", "front", "left", "right" },
    Callback = function(val)
        config.target.attackPosition = val
        pcall(function()
            if getgenv()._NOIR_HEMA and getgenv()._NOIR_HEMA.CFG then
                getgenv()._NOIR_HEMA.CFG.RagePos = tostring(val)
            end
        end)
        syncAtkDeps()
...
attackUnderDep = rageui.ragebottab:AddDependencyBox()
...
attackCustomDep = rageui.ragebottab:AddDependencyBox()
function onAtkCustom()
...
function getHitNotifScreenPosition()
...
function applyHitNotifRootPosition()
    local pos = getHitNotifScreenPosition()
    hitNotifRoot.Position = UDim2.fromOffset(math.floor(pos.X), math.floor(pos.Y))
end
function hnEaseOutCubic(t)
    return 1 - (1 - t) ^ 3
end
function hnEaseInCubic(t)
    return t * t * t
end
function hnEaseOutBack(t)
    local c1 = 1.70158
...
function sampleHitNotifAnim(style, t, isOut)
    t = math.clamp(t, 0, 1)
...
function applyHitNotifVisual(entry, alpha, ox, oy, scale)
...
function measureHitNotifBox(text, textSize)
...
function removeHitNotifEntry(index)
...
function formatHitNotifLine(targetName, dmg, bodyPart)
    return string.format(
...
function createHitNotifBox(text)
    local hn = config.hitNotifications
...
function pushHitNotification(targetName, dmg, bodyPart)
...
function getPlayerFromHitPart(hitPart)
    if not hitPart then
        return nil, nil, nil
    end
...
function shouldNotifyPlayerHit(plr)
    if not config.hitNotifications.enabled or not plr or plr == player then
        return false
    end
    -- only notify when *we* recently damaged this player
    local hum = plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
    if hum and localHitTargets then
        local hit = localHitTargets[hum]
        if hit and (tick() - (hit.hitAt or 0) <= 2.5) then
            return true
        end
    end
    local t = ragePerf and ragePerf.lastHitAtByPlayer and ragePerf.lastHitAtByPlayer[plr]
    local lastShot = getgenv().InstanceCombatLastShotAt or 0
    if t and (tick() - t) <= 2.0 and (tick() - lastShot) <= 2.0 then
        return true
    end
    return false
end
function tryClaimHitDamage(plr, hum, bodyPart, allowFallback)
...
function resolveQueuedHitNotif(plr, job)
    local hum = job.hum
...
function enqueueHitNotifResolve(plr, hum, bodyPart)
...
function pollHitNotifHealth()
    ...
    for _, plr in ipairs(players:GetPlayers()) do
        ...
        -- only track / notify players we ourselves hit recently
        local localHit = localHitTargets and localHitTargets[hum]
        if not localHit or (tick() - (localHit.hitAt or 0) > 2.5) then
        ...
        if cur < last - 0.01 then
            local bodyPart = (localHit and localHit.bodyPart) or (ragePerf.hitPartByPlayer and ragePerf.hitPartByPlayer[plr]) or (config.target and config.target.hitpart) or "Body"
...
function recordLocalHitTarget(plr, hum, bodyPart)
    if not plr or not hum or plr == player then
        return
    end
    localHitTargets[hum] = {
...
function notifyProjectileImpact(hitPart)
    if not hitPart then
        return
    end
...
function onTargetHealthChanged(plr, newHp)
    if not config.hitNotifications.enabled then
        return
    end
...
function unbindHitNotifPlayer(plr)
...
function bindHitNotifCharacter(plr, char)
...
function bindHitNotifPlayer(plr)
    if plr == player then
        return
    end
    if hitNotifCharConn[plr] then
...
function updateHitNotifications(dt)
    local hn = config.hitNotifications
...
noirTabYield("角色 · 個人資料")
frame = Tabs.Character:AddRightGroupbox('Profile')
...
FrameScriptsufhner = Tabs.Character:AddLeftTabbox()
nnTab = FrameScriptsufhner:AddTab("名稱")
skinTab = FrameScriptsufhner:AddTab("皮膚")
...
function handlenewdescendant(d)
    if not config.profile.namespoof.enabled or checkupdating then return end
...
FrameScriptStats = Tabs.Character:AddLeftTabbox()
fpsTab = FrameScriptStats:AddTab("FPS")
msTab = FrameScriptStats:AddTab("延遲")
regionTab = FrameScriptStats:AddTab("地區")
...
noirTabYield("角色 · 移動")
v2 = Tabs.Character:AddRightGroupbox('移動')
...
slfMtrlTab = characterSection:AddTab("自身材質")
animPlayerTab = characterSection:AddTab("動畫播放")
SlfMtrl = {
    enabled = false,
...
function SlfMtrlGetCharacter()
    return p.Character
end
function SlfMtrlGetBodyParts()
...
function SlfMtrlRecordOriginal(part)
...
function SlfMtrlSetEnabled(state)
...
function AnimPlayerGetAnimator()
...
function AnimPlayerPlay()
    ...
    if not ok or not track then
        NoirNotify("failed to load animation", 2)
        return
    end
...
function resetHeadBackwardsMotors()
...
fighter_controller = instanceSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.FighterController)
local_fighter = fighter_controller.LocalFighter
underground_enabled = false
camera_controller = instanceSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.CameraController)
underground_oldpos = nil
function getFloorBelowPosition(pos)
    local rayOrigin = pos
...
function flushAntiAimMovementState()
    underground_oldpos = nil
...
noirTabYield("戰鬥 · 反自瞄")
antiaimbox = Tabs.Combat:AddLeftGroupbox('反自瞄')
...
noirTabYield("雜項 · 自動排隊")
v4 = Tabs.Misc:AddRightGroupbox('自動排隊')
queuemode = "1v1"
ranked = false
queueenabled = false
queuethread = nil
...
MiscUnlockGroup = Tabs.Misc:AddLeftGroupbox('Unlock')
...
function restoreHiddenHead(char)
...
function hideCharacterHead(char)
    if not char then return end
...
deviceBox = Tabs.Misc:AddRightGroupbox('Device Spoofer')
...
slideBox = Tabs.Misc:AddRightGroupbox('滑鏟加速')
...
function qolLightingOverrideActive(ls, key)
...
qolBox = Tabs.Misc:AddLeftGroupbox('QOL')
...
getAspectStretch = getgenv().InstanceGetAspectStretch
worldToEspScreen = getgenv().InstanceWorldToScreenEsp or getgenv().InstanceWorldToScreen
espScreenAnchor = getgenv().InstanceScreenAnchor
...
ConfigBox = _G.Config.Box
ESPObjects = _G.ESPObjects
...
function ensureTextGradient(label, color1, color2)
...
function findBonePart(char, name)
...
function updatecham(player, box, char)
...
function drawskeleton(player, box, char, hum, size, position)
...
function drawhealthbar(player, box, hum, size, position, dt)
...
function drawbox(player, box, size, position)
...
function drawfill(player, box, size, position)
...
function drawglow(player, box, size, position)
...
function drawtext(player, box, size, position, distance)
...
function drawtracers(player, box, root)
...
ESP_RENDER_BIND = "InstanceESPUpdate"
_espMasterWasOn = false
function stepEspUpdate(dt)
...
noirTabYield("視覺 · 透視")
v67 = Tabs.Visuals:AddLeftGroupbox("透視")
...
function resolveUtilImage(modelName)
...
function trackUtilDrawingText(obj)
...
utilespobjects = {}
function makedrawtext(text, color)
...
function makedrawimage(imageId)
...
function getUtilTrackTarget(obj)
...
function utilMatchesFilter(obj)
...
function removeespobjsfor(model)
...
origvalues = {}
updateconn = nil
extraColorCorrection = nil
bloomOrigCache = {}
bloomTracked = {}
function trackBloomEffect(bloom)
...
function restoreBloomOriginals()
...
noirTabYield("視覺 · 燈光")
lightbox = Tabs.Visuals:AddRightGroupbox('燈光')
...
getgenv().crosshair = {
    ...
    length = 10,
    radius = 11,
    crosshair_color = Color3.fromRGB(235, 235, 235),
    color1 = Color3.fromRGB(235, 235, 235),
...
function gradcolor(position, rotation, color1, color2, color3)
...
ammoLabel = getgenv().InstanceTrackDrawingText(Drawing.new("Text"))
ammoLabel.Size = 13
ammoLabel.Font = 2
...
function getammo(t)
    -- ragebot calls getammo() with no args; crosshair passes os.clock()
    t = tonumber(t) or os.clock()
    if ammoUiCache.frame and ammoUiCache.frame.Parent and ammoUiCache.frame.Visible then
        local reserveLabel = ammoUiCache.reserve
        local ammoLabel = ammoUiCache.ammo
        local reserve = 0
        local ammo = 0
        pcall(function()
            if reserveLabel and reserveLabel.Text then
                reserve = tonumber(reserveLabel.Text:match("%d+")) or 0
            end
            if ammoLabel and ammoLabel.Text then
                ammo = tonumber(ammoLabel.Text:match("%d+")) or 0
            end
        end)
        return reserve, ammo
    end
    if t < (tonumber(ammoUiCache.rescanAt) or 0) then return 0, 0 end
...
noirTabYield("視覺 · 準心")
vcrosshair = Tabs.Visuals:AddRightGroupbox("準心")
...
RunService = game:GetService("RunService")
Players = game:GetService("Players")
atmotabbox = Tabs.World:AddRightTabbox()
atmotab = atmotabbox:AddTab('atmosphere')
ambienceTab = atmotabbox:AddTab('ambience')
...
noirTabYield("世界 · 音效")
killSoundsTab = Tabs.World:AddRightGroupbox('擊殺音效')
...
Lighting = game:GetService("Lighting")
RunService = game:GetService("RunService")
skyboxTabbox = Tabs.World:AddLeftTabbox()
skyTab = skyboxTabbox:AddTab('skybox')
aspectTab = skyboxTabbox:AddTab('aspect ratio')
...
skyboxconn = nil
deleteskyboxconn = nil
customSky = nil
origsky = nil
storedorigsky = nil
function storeorigsky()
...
function removecustomsky()
    if customSky then
        customSky:Destroy()
        customSky = nil
    end
end
function skyboxupdater()
...
ASPECT_RATIO_BIND = "InstanceAspectRatio"
...
cam = workspace.CurrentCamera
heightoffset = Vector3.new(0, 20, 0)
function applyparticle(p, cfg)
...
pcall(function() if getgenv()._NOIR_UI_GATE then Library:SetWatermark('instance') end end)
...
kbList = getgenv().InstanceKeybindList
KB_ROW_H = 18
KB_PAD = 8
KB_TITLE_H = 28
KB_HEADER_H = 36
...
function resolveKbFeatureName(idx, opt)
...
function kbTheme(name, fb)
    if Library and Library[name] then return Library[name] end
    return fb
end
function kbTextWidth(text, textSize)
...
function kbCalcPanelWidth(rows)
...
applyKbListTheme = function()
    ...
    local accent = kbTheme("AccentColor", Color3.fromRGB(235, 235, 235))
...
function closeKeyPickerModeMenus()
    if not Library or not Library.ScreenGui then return end
...
MenuGroup:AddLabel('選單快捷鍵'):AddKeyPicker('MenuKeybind', { 
    Default = 'RightShift', 
    NoUI = true, 
    Text = '選單快捷鍵' 
})
...
MenuGroup:AddDropdown('InstanceLanguage', {
    Text = '語言',
    Values = { 'chinese', 'english', 'korean', 'spanish' },
    Default = getgenv().InstanceLanguage or 'chinese',
    Callback = function(val)
        getgenv().InstanceLanguage = val
        if getgenv().InstanceReloadLanguageFont then
            pcall(getgenv().InstanceReloadLanguageFont, val)
        end
...
(function()
ServerGroup = Tabs['UI Settings']:AddRightGroupbox('伺服器')
HttpService = game:GetService("HttpService")
TeleportService = game:GetService("TeleportService")
Players = game:GetService("Players")
LocalPlayer = Players.LocalPlayer
function serverlist(cursor)
...
ServerGroup:AddButton('Rejoin Server', function()
    if game.JobId ~= "" then
        NoirNotify("Rejoining server...", 3)
        TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, LocalPlayer)
    else
        NoirNotify("No valid JobId.", 3)
    end
end)
ServerGroup:AddButton('Server Hop (Random)', function()
    local servers = allservers()
    if #servers == 0 then
        NoirNotify("No servers found.", 3)
        return
    end
    local s = servers[math.random(#servers)]
    NoirNotify("Hopping to server (" .. s.playing .. "/" .. s.maxPlayers .. " players)", 5)
...
ServerGroup:AddButton('Join Lowest Players Server', function()
    ...
    local s = servers[1]
    NoirNotify("Joining lowest server (" .. s.playing .. "/" .. s.maxPlayers .. " players)", 5)
    TeleportService:TeleportToPlaceInstance(game.PlaceId, s.id, LocalPlayer)
end)
ServerGroup:AddButton('Join Biggest Server', function()
    ...
    local s = servers[1]
    NoirNotify("Joining biggest server (" .. s.playing .. "/" .. s.maxPlayers .. " players)", 5)
    TeleportService:TeleportToPlaceInstance(game.PlaceId, s.id, LocalPlayer)
end)
end)();
...
animPlayerTab:AddInput("AnimForceID", { Default = "96579993895076", Text = "主動畫 ID", Placeholder = "numbers", Numeric = false, Finished = true })
...
(function()
    ...
    clearAnims()
        ...
        if not animPrimary then
            NoirNotify("animation bypass failed (bad bundle id?)", 3)
            return false
        end
...
function textureAssetMatches(value)
...
function instanceHasTargetTexture(inst)
...
function getInstProperty(inst, prop)
    local ok, value = pcall(function() return inst[prop] end)
    return ok and value or nil
end
function setInstProperty(inst, prop, value)
    pcall(function() inst[prop] = value end)
end
function matchAssetValue(value)
    if not value then return false end
...
function recordTargetTexture(inst)
...
function restoreTargetTexture(inst)
...
function processTargetTextureInstance(inst)
...
function scanTargetTextures()
    for _, inst in ipairs(game:GetDescendants()) do
        if instanceHasTargetTexture(inst) then degradeTextureInstance(inst) end
    end
end
function startTargetTextureFilter()
    scanTargetTextures()
...
function stopTargetTextureFilter()
...
darkTexOrig = {}
darkTexConn = nil
function applyDarkTexturesToInst(inst)
...
function applyTransparentToInst(inst)
...
function refreshAllTransparent()
...
function applyTransparentTextures()
    for _, inst in ipairs(workspace:GetDescendants()) do
...
function restoreTransparentTextures()
...
function updTransparentTextures()
    if _G.Features.TransparentTextures.Enabled then
...
themeManagerThemeUpdate = ThemeManager.ThemeUpdate
...
-- Config loading flag stays true until SaveManager autoload finishes (see end of script)
...
MenuGroup = Tabs["UI Settings"]:AddLeftGroupbox("選單")
MenuGroup:AddToggle("KeybindMenuOpen", { Default = Library.KeybindFrame.Visible, Text = "開啟快捷鍵選單", Callback = function(value) Library.KeybindFrame.Visible = value end})
MenuGroup:AddToggle("ShowCustomCursor", {Text = "自訂游標", Default = false, Callback = function(Value) Library.ShowCustomCursor = Value; pcall(function() local uis=game:GetService("UserInputService"); if Value then uis.MouseIconEnabled = false else uis.MouseIconEnabled = true end end) end})
MenuGroup:AddDivider()
MenuGroup:AddLabel("選單快捷鍵"):AddKeyPicker("MenuKeybind", { Default = "RightShift", NoUI = true, Text = "選單快捷鍵" })
MenuGroup:AddButton("Unload", function() Library:Unload() end)
Library.ToggleKeybind = Options.MenuKeybind

-- Save / Load configs — 純按鈕操作（不依賴下拉表）
pcall(function()
    ThemeManager:SetLibrary(Library)
    ...
    ThemeManager:SetFolder("Noir")
    SaveManager:SetFolder("Noir")
    local placeIdStr = tostring(game.PlaceId)
    pcall(function()
        if SaveManager.SetSubFolder then
            SaveManager:SetSubFolder(placeIdStr)
        end
    end)

    local function configFolderPath()
        return "Noir/settings/" .. placeIdStr
    end

    pcall(function()
        if isfolder and not isfolder("Noir") then makefolder("Noir") end
        if isfolder and not isfolder("Noir/settings") then makefolder("Noir/settings") end
        if isfolder and not isfolder(configFolderPath()) then makefolder(configFolderPath()) end
        if isfolder and not isfolder("Noir/themes") then makefolder("Noir/themes") end
        if SaveManager.BuildFolderTree then SaveManager:BuildFolderTree() end
        if SaveManager.CheckFolderTree then SaveManager:CheckFolderTree() end
    end)
    ...
    local function listConfigFiles()
        ...
        pcall(function()
            local sm = SaveManager:RefreshConfigList()
            if type(sm) == "table" then
                for _, n in ipairs(sm) do
                    n = tostring(n)
                    if n ~= "" and n ~= "none" then
                        local found = false
                        for _, x in ipairs(names) do
                            if x == n then found = true break end
                        end
    ...
    local function setSelected(name)
        ...
        pcall(function()
            if Options and Options.NoirConfigName and Options.NoirConfigName.SetValue then
                Options.NoirConfigName:SetValue(selectedConfig)
            end
        end)
        pcall(function()
            if statusLabel then
                if statusLabel.SetText then
                    statusLabel:SetText(currentLabel())
                elseif statusLabel.SetValue then
                    statusLabel:SetValue(currentLabel())
                end
            end
        end)
    end
    ...
    local function getName()
        local n = Options and Options.NoirConfigName and Options.NoirConfigName.Value
    ...
    local cfgBox = Tabs["UI Settings"]:AddLeftGroupbox("設定檔")
    cfgBox:AddLabel("不使用下拉表 · 請用下方按鈕")
    cfgBox:AddInput("NoirConfigName", {
        Default = "",
        Numeric = false,
        Finished = false,
        Text = "設定檔名稱",
    ...
    statusLabel = cfgBox:AddLabel(currentLabel())

    local function addBtn(text, fn)
        -- support both Linoria AddButton styles
        local ok = pcall(function()
            cfgBox:AddButton({ Text = text, Func = fn })
        end)
        if not ok then
            pcall(function()
                cfgBox:AddButton(text, fn)
            end)
        end
    end

    addBtn("重新整理列表", function()
        local list = listConfigFiles()
        if #list == 0 then
            NoirNotify("沒有檔案 · 路徑: " .. configFolderPath(), 4)
            print("[NOIR] empty config folder", configFolderPath())
            setSelected("")
        else
            NoirNotify("找到 " .. #list .. " 個: " .. table.concat(list, ", "), 5)
            print("[NOIR] configs:", table.concat(list, ", "))
            if selectedConfig == "" then
                setSelected(list[1])
            else
                local still = false
                for _, n in ipairs(list) do if n == selectedConfig then still = true break end end
                if not still then setSelected(list[1]) else setSelected(selectedConfig) end
            end
        end
    end)

    addBtn("上一個設定檔", function()
        local list = listConfigFiles()
        if #list == 0 then
            NoirNotify("沒有設定檔，請先儲存", 3)
            return
        end
        local idx = 1
        for i, n in ipairs(list) do
            if n == selectedConfig then idx = i break end
        end
        idx = idx - 1
        if idx < 1 then idx = #list end
        setSelected(list[idx])
        NoirNotify("已選: " .. list[idx], 2)
    end)

    addBtn("下一個設定檔", function()
        local list = listConfigFiles()
        if #list == 0 then
            NoirNotify("沒有設定檔，請先儲存", 3)
            return
    ...
    addBtn("儲存設定檔", function()
        local name = getName()
        if not name then
            NoirNotify("請先在名稱欄輸入名字", 3)
            return
        end
        pcall(function()
            if isfolder and not isfolder(configFolderPath()) then makefolder(configFolderPath()) end
        ...
        if ok then
            setSelected(name)
            listConfigFiles()
            NoirNotify("已儲存: " .. name, 3)
            print("[NOIR] saved", name, "at", configFolderPath())
        else
            NoirNotify("儲存失敗: " .. tostring(err), 4)
            warn("[NOIR] save failed", err)
        end
    end)

    addBtn("讀取設定檔", function()
        local name = getName()
        if not name then
            NoirNotify("請輸入名稱，或按「下一個設定檔」選擇", 3)
            return
        end
        getgenv().InstanceConfigLoading = true
        local ok, err = pcall(function()
        ...
        if ok then
            setSelected(name)
            NoirNotify("已讀取: " .. name, 3)
            pcall(function()
                if getgenv().InstanceSyncAfterConfigLoad then
                    getgenv().InstanceSyncAfterConfigLoad()
                end
            end)
        else
            NoirNotify("讀取失敗: " .. tostring(err), 4)
            warn("[NOIR] load failed", err)
        end
    end)

    addBtn("刪除選定設定檔", function()
        local name = getName()
        if not name then
            NoirNotify("沒有可刪除的設定檔", 3)
            return
        end
        local ok, err = pcall(function()
        ...
        if ok then
            NoirNotify("已刪除: " .. name, 3)
            setSelected("")
            listConfigFiles()
        else
            NoirNotify("刪除失敗: " .. tostring(err), 3)
        end
    end)

    addBtn("設為自動載入", function()
        local name = getName()
        if not name then
            NoirNotify("請先選擇設定檔", 3)
            return
        end
        local ok, err = pcall(function()
        ...
        if ok then
            NoirNotify("下次啟動自動載入: " .. name, 4)
        else
            NoirNotify("設定失敗: " .. tostring(err), 4)
        end
    end)

    addBtn("取消自動載入", function()
        local ok, err = pcall(function()
            return SaveManager:DeleteAutoLoadConfig()
        end)
        if ok then
            NoirNotify("已取消自動載入", 3)
        else
            NoirNotify("取消失敗: " .. tostring(err), 3)
        end
    end)

    -- 故意不呼叫 BuildConfigSection（其下拉表在多數執行器點不開）

    task.defer(function()
        local list = listConfigFiles()
        if #list > 0 then
            setSelected(list[1])
            NoirNotify("設定檔就緒 · " .. #list .. " 個（用按鈕切換）", 3)
        else
            NoirNotify("設定檔就緒 · 尚無檔案（輸入名稱後按儲存）", 3)
        end
        print("[NOIR] config folder", configFolderPath(), "count", #list)
    end)
    ...
    task.spawn(function()
        ...
        local autoName = nil
        pcall(function()
            autoName = SaveManager:GetAutoloadConfig()
        end)
        if autoName and autoName ~= "" and autoName ~= "none" then
            NoirNotify("自動載入: " .. tostring(autoName), 2)
            getgenv().InstanceConfigLoading = true
            local ok, err = pcall(function()
                return SaveManager:Load(autoName)
            end)
            getgenv().InstanceConfigLoading = false
            if ok then
                setSelected(tostring(autoName))
                NoirNotify("已自動載入: " .. tostring(autoName), 3)
            else
                NoirNotify("自動載入失敗（可手動讀取）", 3)
                warn("[NOIR] autoload:", err)
            end
        end
        listConfigFiles()
        task.wait(0.05)
        ...
        pcall(function()
            if getgenv().InstanceApplyAccentTheme then
                getgenv().InstanceApplyAccentTheme()
            end
        end)
    end)
end)

-- 選單開啟時強制解放滑鼠，讓所有 UI（含按鈕）可點
pcall(function()
    local uis = game:GetService("UserInputService")
    local rs = game:GetService("RunService")
    local bindName = "NoirForceMouseUnlock"
    pcall(function() rs:UnbindFromRenderStep(bindName) end)
    rs:BindToRenderStep(bindName, Enum.RenderPriority.Last.Value, function()
        local open = false
        pcall(function() open = Library and Library.Toggled == true end)
        if open then
            pcall(function()
                uis.MouseBehavior = Enum.MouseBehavior.Default
                if not (Library and Library.ShowCustomCursor) then
                    uis.MouseIconEnabled = true
                end
            end)
        end
...
-- ============================================================
-- HEMA Silent + Rage (replaces Lunara silent/rage fire paths)
-- Source: hippohub Hema-Rivals.lua
-- ============================================================
pcall(function()
    local Players = game:GetService("Players")
    local RS = game:GetService("ReplicatedStorage")
    ...
    local HEMA = getgenv()._NOIR_HEMA or {}
    getgenv()._NOIR_HEMA = HEMA

    HEMA.CFG = HEMA.CFG or {
        Silent = false,
        SilentAuto = false,
        SilentFOV = 200,
        FOV = 200,
        ...
        FireRate = 0.05,
        WallCheck = false,
        TeamCheck = false, -- default off like Lunara silent
        AntiKatana = false,
        RageOn = false,
        RagePos = "random", -- 上下左右超快速隨機
        RageRandomShuffle = true,
        RageDist = 5,
        RageUnder = 8,
        RageAutoShoot = true,
        RageReturn = true,
    ...
    local lastFire, lastSilentAt = 0, 0
    local rageOrigCF, rageTargetPlr = nil, nil
    local lastRageAt = 0
    ...
    local function sameTeam(plr)
        ...
        return checkteammate(plr)
    end
    local function isEnemy(plr)
        if not plr or plr == lp then return false end
        if CFG.TeamCheck and sameTeam(plr) then return false end
    ...
    local function ensureMods()
        ...
        pcall(function()
            ...
            if (not Utility or not Utility.EncodeCFrame) and m:FindFirstChild("Utility") then
                local ok, mod = pcall(require, m.Utility)
                if ok then Utility = mod end
            end
            if (not EnumLibrary or not EnumLibrary.ToEnum) and m:FindFirstChild("EnumLibrary") then
    ...
    local function isKatanaUser(plr)
        if not plr or not plr.Character then return false end
        local found = false
        pcall(function()
            for _, d in ipairs(plr.Character:GetChildren()) do
                local n = string.lower(tostring(d.Name))
                if n:find("katana", 1, true) then found = true break end
            end
            local tool = plr.Character:FindFirstChildOfClass("Tool")
            if tool and string.lower(tool.Name):find("katana", 1, true) then found = true end
        end)
        return found
    end

    -- HEMA trySilent (Lunara packet)
    local function trySilent(part, forceRage)
        ...
        if CFG.AntiKatana and tplr and isKatanaUser(tplr) then return end
        ...
        lastSilentAt = now
        if tplr and recordLocalHitTarget then
            local hum2 = model and model:FindFirstChildOfClass("Humanoid")
            if hum2 then
                pcall(recordLocalHitTarget, tplr, hum2, part.Name)
            end
        end
    end

    local function getTarget(fovOverride)
        local cam = workspace.CurrentCamera
        ...
        local maxFov = fovOverride or CFG.SilentFOV or CFG.FOV or 200
        local best, bestD = nil, maxFov
        local me = hrp()
        local maxDist = CFG.MaxDist or 500
        for _, plr in ipairs(Players:GetPlayers()) do
            if isEnemy(plr) and plr.Character and lunaraValidChar(plr.Character) then
                local part = hitpartfromname(plr.Character, CFG.Part)
                if part then
                    if (not me) or (part.Position - me.Position).Magnitude <= maxDist then
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            if isEnemy(plr) and plr.Character then
                ...
                if root and hum2 and hum2.Health > 0 then
                    if CFG.AntiKatana and isKatanaUser(plr) then
                        -- skip
                    else
                        local part = hitpartfromname(plr.Character, CFG.Part)
    ...
    -- 隨機上下左右：每幀換一個方向，超快
    local _rageRandDirs = { "up", "down", "left", "right" }
    local _rageRandIdx = 1
    local function rageAttackCF(plr)
        ...
        if mode == "under" or mode == "下面" or mode == "down" then
            pos = root.Position + Vector3.new(0, -under, 0)
        elseif mode == "above" or mode == "上面" or mode == "up" then
            pos = root.Position + Vector3.new(0, under, 0)
        elseif mode == "front" or mode == "前面" then
            pos = (cf * CFrame.new(0, 0, -dist)).Position
        elseif mode == "left" or mode == "左" then
            pos = (cf * CFrame.new(-dist, 0, 0)).Position
        elseif mode == "right" or mode == "右" then
            pos = (cf * CFrame.new(dist, 0, 0)).Position
        elseif mode == "random" or mode == "隨機" or mode == "random_udlr" or mode == "default" then
            -- 上下左右循環／亂跳，超快速
            if CFG.RageRandomShuffle ~= false then
                _rageRandIdx = math.random(1, #_rageRandDirs)
            else
                _rageRandIdx = (_rageRandIdx % #_rageRandDirs) + 1
            end
            local dir = _rageRandDirs[_rageRandIdx]
            local d = math.max(dist, under, 4)
            if dir == "up" then
                pos = root.Position + Vector3.new(0, d, 0)
            elseif dir == "down" then
                pos = root.Position + Vector3.new(0, -d, 0)
            elseif dir == "left" then
                pos = (cf * CFrame.new(-d, 0, 0)).Position
            else
                pos = (cf * CFrame.new(d, 0, 0)).Position
            end
        else
    ...
    local function stepRage()
        ...
        if atk then
            pcall(function() r.CFrame = atk end)
        end
        if CFG.RageAutoShoot then
            local part = hitpartfromname(plr.Character, CFG.Part)
            if part then
                trySilent(part, true)
            end
    ...
    -- Disable old Lunara fire / orbit paths (HEMA takes over)
    pcall(function()
        firesilent = function() end
    end)
    pcall(function()
        autoshoot = function() end
    end)
    pcall(function()
        -- prevent old rage orbit/settarget from fighting HEMA teleport
        if type(settarget) == "function" then
            settarget = function(char)
                -- HEMA handles targeting; keep character nil for old path
                if config and config.target then
                    config.target.enabled = false
                    config.target.character = nil
                    config.target.rageMasterOn = (CFG.RageOn == true)
                end
            end
        end
    ...
    -- Sync from Linoria toggles when present
    local function syncFromUI()
        pcall(function()
            if Toggles then
                if Toggles.SilentAim then CFG.Silent = Toggles.SilentAim.Value == true end
                if Toggles.SilentAutoShoot then CFG.SilentAuto = Toggles.SilentAutoShoot.Value == true end
                if Toggles.SilentWallCheck then CFG.WallCheck = Toggles.SilentWallCheck.Value == true end
                if Toggles.AntiKatana then CFG.AntiKatana = Toggles.AntiKatana.Value == true end
                if Toggles.TargetOn then CFG.RageOn = Toggles.TargetOn.Value == true end
                if Toggles.AutoTarget then end -- HEMA always finds nearest when RageOn
            end
            if Options then
                if Options.HitChance then CFG.HitChance = tonumber(Options.HitChance.Value) or CFG.HitChance end
                if Options.HitPartDropdown then CFG.Part = tostring(Options.HitPartDropdown.Value or CFG.Part) end
                if Options.TargetPart then CFG.Part = tostring(Options.TargetPart.Value or CFG.Part) end
                if Options.SilentMaxRange then CFG.MaxDist = tonumber(Options.SilentMaxRange.Value) or CFG.MaxDist end
                if Options.AutoTargetAttackPos then CFG.RagePos = tostring(Options.AutoTargetAttackPos.Value or CFG.RagePos) end
            end
            if silentAim then
                if silentAim.fovRadius then CFG.SilentFOV = silentAim.fovRadius end
                if silentAim.hitPart then CFG.Part = silentAim.hitPart end
                if silentAim.hitChance then CFG.HitChance = silentAim.hitChance end
                if silentAim.wallCheck ~= nil then CFG.WallCheck = silentAim.wallCheck end
                if silentAim.autoShoot ~= nil then CFG.SilentAuto = silentAim.autoShoot end
                if silentAim.enabled ~= nil then CFG.Silent = silentAim.enabled end
                if silentAim.maxRange then CFG.MaxDist = silentAim.maxRange end
            end
            if config and config.target then
                if config.target.rageMasterOn ~= nil then CFG.RageOn = config.target.rageMasterOn or CFG.RageOn end
                if config.target.hitpart then CFG.Part = config.target.hitpart end
                if config.target.attackPosition then CFG.RagePos = config.target.attackPosition end
                if config.target.autoshoot ~= nil then CFG.RageAutoShoot = config.target.autoshoot end
                -- keep old Lunara rage orbit off
                config.target.enabled = false
                config.target.character = nil
            end
        end)
    ...
    -- Main combat loop (HEMA style)
    if HEMA._conn then pcall(function() HEMA._conn:Disconnect() end) end
    HEMA._conn = RunService.RenderStepped:Connect(function()
        if getgenv().HEMA_KILL then return end
        -- pause when menu open optional
        local menuOpen = false
        pcall(function() menuOpen = Library and Library.Toggled == true end)
        if menuOpen then return end
        syncFromUI()
        if not CFG.Silent and not CFG.SilentAuto and not CFG.RageOn then return end
        ensureMods()
        if CFG.Silent or CFG.SilentAuto then
            local t = getTarget(CFG.SilentFOV)
            if t then trySilent(t, false) end
        end
        if CFG.RageOn then
            local now = tick()
            local mode = tostring(CFG.RagePos or ""):lower()
            local isRand = (mode == "random" or mode == "隨機" or mode == "random_udlr" or mode == "default")
            -- 隨機模式：幾乎每幀換位（超快速）；其他約 0.03s
            local interval = isRand and 0.0 or 0.03
            if now - lastRageAt >= interval then
                lastRageAt = now
                stepRage()
            end
        else
    ...
    HEMA.trySilent = trySilent
    HEMA.getTarget = getTarget
    HEMA.stepRage = stepRage
    HEMA.syncFromUI = syncFromUI
    print("[NOIR] HEMA Silent + Rage loaded (Hema-Rivals core)")
    pcall(function()
        if NoirNotify then NoirNotify("已切換為 HEMA 靜默/狂暴", 3) end
    end)
end)
...
-- ============================================================
-- 等「競爭者 / 遊戲 UI」真正出現並穩定後，才允許 NOIR 選單
-- ============================================================
task.spawn(function()
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
    ...
    local ROBLOX_CORE = {
        ...
        PurchasePrompt = true,
        TeleportGui = true,
        ScreenTimeShutdown = true,
        ExperienceShutdown = true,
    ...
    local function isNoirGui(inst)
        ...
        if n:find("noir", 1, true) or n:find("linoria", 1, true) or n:find("instance", 1, true) then
            return true
        end
        if Library and Library.ScreenGui and inst == Library.ScreenGui then
            return true
        end
        return false
    end

    local function listForeignCoreGuis()
        local list = {}
        pcall(function()
            for _, c in ipairs(CoreGui:GetChildren()) do
                if c:IsA("ScreenGui") and not isNoirGui(c) then
                    local name = c.Name
                    if not ROBLOX_CORE[name] then
    ...
    local function forceNoirClosed()
        pcall(function()
            getgenv()._NOIR_UI_GATE = false
            if Library then
                Library.Toggled = false
                if Library.KeybindFrame then Library.KeybindFrame.Visible = false end
                if Library.SetWatermarkVisibility then
    ...
    forceNoirClosed()
    print("[NOIR] waiting for competitor / game UI before showing menu...")

    -- 1) 遊戲基本載入
    local deadline = os.clock() + 60
    while os.clock() < deadline do
        if game:IsLoaded() and lp and lp.Parent then break end
        forceNoirClosed()
        task.wait(0.1)
    end

    -- 2) 等 PlayerGui 有遊戲 UI
    pcall(function()
        lp:WaitForChild("PlayerGui", 25)
    end)
    local tPg = os.clock()
    while os.clock() - tPg < 30 do
        forceNoirClosed()
        if playerGuiReady() then break end
        task.wait(0.2)
    end

    -- 3) 核心：等到至少 1 個「非 Roblox、非 NOIR」的 CoreGui ScreenGui
    --    （通常就是競爭者腳本的 UI），再等數量穩定
    local foreignStable = 0
    local lastForeignKey = ""
    local foundForeign = false
    local waitDeadline = os.clock() + 40
    while os.clock() < waitDeadline do
        forceNoirClosed()
        local foreign = listForeignCoreGuis()
        local key = table.concat(foreign, "|")
        if #foreign >= 1 then
            foundForeign = true
            if key == lastForeignKey then
                foreignStable = foreignStable + 1
                -- 穩定約 3 秒（15 * 0.2）
                if foreignStable >= 15 then
                    print("[NOIR] competitor UI detected:", key)
                    break
                end
            else
                foreignStable = 0
                lastForeignKey = key
                print("[NOIR] foreign UI changed:", key)
            end
        else
            foundForeign = false
            foreignStable = 0
    ...
    if not foundForeign then
        -- 超時仍沒偵測到競爭者：再等遊戲 UI 穩定後繼續，避免卡死
        print("[NOIR] no foreign CoreGui found in time · fallback to PlayerGui settle")
        local lastN, stable = -1, 0
        local fbDeadline = os.clock() + 8
        while os.clock() < fbDeadline do
            forceNoirClosed()
            local n = 0
            pcall(function()
                local pg = lp:FindFirstChild("PlayerGui")
                if pg then n = #pg:GetChildren() end
            end)
            if n == lastN and n > 0 then
                stable = stable + 1
                if stable >= 10 then break end
            else
                stable = 0
                lastN = n
            end
        ...
        -- 競爭者已穩定，再多留一點時間給對方初始化
        task.wait(1.0)
        forceNoirClosed()
    end

    -- 4) 放行 NOIR UI（仍先保持關閉，只是允許顯示）
    getgenv()._NOIR_UI_GATE = true
    pcall(function()
        if Library and Library.ScreenGui then
    ...
    pcall(function()
        if NoirNotify then
            if foundForeign then
                NoirNotify("NOIR 已就緒（已等競爭者 UI）· RightShift 開選單", 4)
            else
                NoirNotify("NOIR 已就緒 · RightShift 開選單", 4)
            end
        end
    end)
    print("[NOIR] UI gate open · foreign=", tostring(foundForeign), "· RightShift to open")
end)
